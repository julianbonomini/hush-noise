...
```
go/     ← Go reference implementation; module path github.com/julianbonomini/hush-noise/go
rust/   ← Rust portable implementation; crate name hush-noise (crates.io)
```
...
The Go module path is `github.com/julianbonomini/hush-noise/go`. The rename from the original root-level path happens while there are zero external consumers — the only moment this costs nothing.

The Rust crate is named `hush-noise` in `Cargo.toml` but is not published to crates.io until the implementation is complete and the project is open-sourced. The name is available and will be claimed on first publish.
